...
public class SimpleKeyedQueueChannelBus implements KeyedQueueChannelBus {
    private final Map<ObjectChannel, Map<Object, Queue<?>>> channels;
    ...
    @Override
    public <K, V> void forEach(ObjectChannel channel, BiConsumer<? super K, Queue<V>> consumer) {
        this.<K, V>stream(channel).forEach(entry -> consumer.accept(entry.key(), entry.value()));
    }

    @SuppressWarnings("unchecked")
    @Override
    public <K, V> Stream<KeyedQueueChannelEntry<K, V>> stream(ObjectChannel channel) {
    ...
    @Override
    public <K, V> void forEachDrain(ObjectChannel channel, BiConsumer<? super K, Queue<V>> consumer) {
        this.<K, V>drain(channel).forEach(entry -> consumer.accept(entry.key(), entry.value()));
    }

    @SuppressWarnings("unchecked")
    @Override
    public <K, V> Stream<KeyedQueueChannelEntry<K, V>> drain(ObjectChannel channel) {
